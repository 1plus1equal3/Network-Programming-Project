...
void getFullImagePath(){
    ...
    for(int i = 0; i < question_num; i++){
        ...
        sprintf((full_media_path)[i], "/home/duy/NetworkProgramming/Project/main/main_reconstruct/client/source/image/%s", media_path);
    }}
    else if (media_type == 1){
        for(int i = 0; i < question_num; i++){
        char *media_path = (*question_list)[i].image_path;
        sprintf((full_media_path)[i], "/home/duy/NetworkProgramming/Project/main/main_reconstruct/client/source/soundtrack/%s", media_path);
    }}
}

void resize_image(char *media_path) {
    char cmd[1000];
    sprintf(cmd, "/home/duy/NetworkProgramming/Project/main/main_reconstruct/client/resize_img.sh %s", media_path);
    system(cmd);
}

void deleteImageAfterUsed() {
    char cmd[1000];
    sprintf(cmd, "rm -r %s", "/home/duy/NetworkProgramming/Project/main/main_reconstruct/client/source/image/*");
    system(cmd);
}

void deleteSoundAfterUsed() {
    char cmd[1000];
    sprintf(cmd, "rm -r %s", "/home/duy/NetworkProgramming/Project/main/main_reconstruct/client/source/soundtrack/*");
    system(cmd);
}
...
int startImageQuestionInterface() {
    ...
    viewerOpen = 1;
    gtk_init(NULL, NULL);
    builder = gtk_builder_new_from_file("/home/duy/NetworkProgramming/Project/main/main_reconstruct/client/image_UI.glade");
...
int startSoundQuestionInterface() {
    ...
    viewerOpen = 1;
    gtk_init(NULL, NULL);
    builder = gtk_builder_new_from_file("/home/duy/NetworkProgramming/Project/main/main_reconstruct/client/sound_UI.glade");
    GtkCssProvider *provider;
    provider = gtk_css_provider_new();
    GdkDisplay *display;
    GdkScreen *screen;
